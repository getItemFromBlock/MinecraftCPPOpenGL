...
unsigned int GenerateChunkVertices(std::vector<Core::Util::Vertice>* vertices, std::vector<Core::Util::TangentVertice>* vOut, std::vector<unsigned int>* iOut)
{
	unsigned int count = 0;
	for (unsigned int i = 0; i < vertices->size() / 3; i++)
	{
		...
		for (int n = 0; n < 3; n++)
		{
			data[n].pos = vertices->at(i * 3llu + n).pos;
		...
		{
			data[n].tang = tangent;
			iOut->push_back((unsigned int)vOut->size());
			vOut->push_back(data[n]);
			count++;
		}
	}
	return count;
...
void Core::Util::ModelLoader::LoadChunk(World::ChunkModel* in, std::vector<Vertice>* vertices)
{
	bool empty = in->mesh.GetVerticesCount() == 0;
	in->mesh.loaded = false;
	std::vector<unsigned int> indexesOut;
	if (empty)
	{
		glGenBuffers(1, &in->mesh.VBO);
		glGenBuffers(1, &in->mesh.EBO);
		glGenVertexArrays(1, &in->mesh.VAO);
	}
	std::vector<unsigned int> indexes;
	std::vector<TangentVertice> verts;
	in->mesh.indexCount = GenerateChunkVertices(vertices, &verts, &indexes);
	// bind the Vertex Array Object first, then bind and set vertex buffer(s), and then configure vertex attributes(s).
	glBindVertexArray(in->mesh.VAO);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, in->mesh.EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, indexes.size() * sizeof(unsigned int), indexes.data(), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, in->mesh.VBO);

	glBufferData(GL_ARRAY_BUFFER, verts.size() * sizeof(TangentVertice), verts.data(), GL_STATIC_DRAW);
	// position attribute
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(TangentVertice), (void*)0);
	glEnableVertexAttribArray(0);
	// normal attribute
