...
namespace Networking
{
	...
	{
		std::copy(&other.mStorage, &other.mStorage + 1, &mStorage);
		mType = other.mType;
		mPort = other.mPort;
	}
	...
	Address::Address(const std::string& ip, uint16_t port) : mPort(port)
	{
	...
	Address Address::Any(Type type, u16 port)
	{
		switch (type)
		{
		case Type::IPv4:
	...
	Address Address::Loopback(Type type, u16 port)
	{
		switch (type)
		{
		case Type::IPv4:
		...
		case Type::IPv6:
		{
			sockaddr_storage storage{ 0 };
			sockaddr_in6& addr = reinterpret_cast<sockaddr_in6&>(storage);
			addr.sin6_addr = IN6ADDR_LOOPBACK_INIT; // I hope thats gonna work
			addr.sin6_port = htons(port);
			addr.sin6_family = AF_INET6;
			return Address(reinterpret_cast<sockaddr_storage&>(addr));
		}
	...
	{
		char portBuffer[6];
		snprintf(portBuffer, 6, "%lu", mPort);
		return (address() + ":" + portBuffer);
	}

	void Address::set(const sockaddr_storage& src)
	{
		std::copy(&src, &src + 1, &mStorage);
	...
	std::string Address::address() const
	{
		if (mType == Type::None)
			return "<None>";
		static constexpr int MaxBufferSize = INET_ADDRSTRLEN > INET6_ADDRSTRLEN ? INET_ADDRSTRLEN : INET6_ADDRSTRLEN;
		char buffer[MaxBufferSize];
		// Utilisation de const_cast à cause de certaines API Windows... la variable n’est pas modifiée donc c’est OK
	...
	int Address::recvFrom(SOCKET sckt, u8* buffer, size_t bufferSize)
	{
		sockaddr_storage storage{ 0 };
